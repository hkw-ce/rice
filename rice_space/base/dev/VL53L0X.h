...
// Use RT-Thread tick to provide millisecond tick count instead of HAL_GetTick
#include "rtthread.h"

// Record the current time to check an upcoming timeout against (ms)
#define startTimeout() (g_timeoutStartMs = (uint16_t)(((uint32_t)rt_tick_get() * 1000U) / RT_TICK_PER_SECOND))

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (g_ioTimeout > 0 && ((uint16_t)((((uint32_t)rt_tick_get() * 1000U) / RT_TICK_PER_SECOND)) - g_timeoutStartMs) > g_ioTimeout)
